...
class InternshipApplicationAdmin(admin.ModelAdmin):
    list_display = ['full_name', 'email', 'university', 'status', 'is_read', 'created_at']
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('full_name', 'email', 'phone', 'university')
        }),
        ('Application Details', {
            'fields': ('skills', 'experience', 'portfolio_url', 'resume_url')
        }),
        ('Status', {
            'fields': ('status', 'is_read', 'notes')
        }),
        ('Metadata', {
            'fields': ('created_at',),
            'classes': ('collapse',)
...
class DeveloperApplicationAdmin(admin.ModelAdmin):
    ...
    ordering = ['-created_at']
    readonly_fields = ['created_at']
    actions = ['mark_as_read', 'mark_as_pending', 'mark_as_reviewed', 'mark_as_rejected']
    fieldsets = (
...
class JoinApplicationAdmin(admin.ModelAdmin):
    list_display = ['full_name', 'email', 'role_interested', 'status', 'is_read', 'created_at']
    list_filter = ['status', 'is_read', 'created_at']
    search_fields = ['full_name', 'email', 'role_interested']
    ordering = ['-created_at']
    readonly_fields = ['created_at']
    actions = ['mark_as_read', 'mark_as_pending', 'mark_as_reviewed', 'mark_as_accepted', 'mark_as_rejected']
    fieldsets = (
        ('Personal Information', {
            'fields': ('full_name', 'email', 'role_interested')
        }),
        ('Application', {
            'fields': ('why_join', 'status', 'is_read')
        }),
        ('Metadata', {
            'fields': ('created_at',),
            'classes': ('collapse',)
